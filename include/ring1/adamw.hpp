...
inline float trust_region_for_loss(float loss) {
    ...
    if (loss >= 3.0f) return 0.12f + (0.35f - 0.12f) * (6.0f - loss) / 3.0f; // 6->0.12, 3->0.35
    if (loss >= 1.0f) return 0.35f + (0.60f - 0.35f) * (3.0f - loss) / 2.0f; // 3->0.35, 1->0.60
    return 0.60f;
}
...
public:
    ...
    float penalty_factor = 0.80f;      ///< Active penalization multiplier
