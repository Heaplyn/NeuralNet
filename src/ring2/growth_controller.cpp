    ...
    GrowthReport GrowthController::record_epoch_loss(float loss, NeuralNet &net)
    {
        ...
        if (config.use_taylor_forecast && loss_history.size() >= 3)
        {
            ...
            if (tj.valid)
            {
                ...
                forecast_stagnation = (forecast_net > -config.stagnation_threshold * max(1.0f, loss)) && (forecast_conf >= config.forecast_confidence_gate);
                if (config.forecast_uncaps_width && forecast_conf >= config.forecast_confidence_gate)
                {
                    ...
                    float stall = clamp(0.5f + 0.5f * tanhf(forecast_net * 7.0f), 0.0f, 1.3f);
        ...
        for (size_t i = 0; i < half; ++i)
            old_avg += loss_history[i];
        old_avg /= half;

        float new_avg = 0.0f;
        for (size_t i = half; i < loss_history.size(); ++i)
            new_avg += loss_history[i];
        new_avg /= (half);
