...
float MultiHeadAttention::get_alibi_slope(size_t head_idx) const {
    ...
    return pow(2.0f, -power) * 0.25f; // Softened distance falloff
}
        ...
        for (int h = 0; h < static_cast<int>(num_heads); ++h) {
            ...
            for (size_t i = 0; i < T; ++i) {
                ...
                for (size_t j = 0; j <= i; ++j) {
                    ...
                    float score = (dot * scale) - (slope * distance);
                    if (std::isnan(score) || std::isinf(score)) score = -50.0f;
                    scores(i, j) = score;
                    if (score > row_max) row_max = score;
                }
                if (row_max < -1e20f) row_max = 0.0f;
