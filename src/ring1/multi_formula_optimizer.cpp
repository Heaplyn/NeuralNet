...
float MultiFormulaKernel::execute_update_formula(
    ...
    if (std::isnan(m_hat) || std::isinf(m_hat)) m_hat = 0.0f;
    if (std::isnan(v_hat) || std::isinf(v_hat) || v_hat < 0.0f) v_hat = 0.0f;
    if (std::isnan(f_hat) || std::isinf(f_hat) || f_hat < 0.0f) f_hat = 0.0f;
